...
MODEL_SPEC = spec.ModelSpec(
    ...
    inputs=[
        ...
        spec.SingleBandRasterInput(
            ...
            about=gettext("A map of LULC for the baseline scenario, which must occur prior to the alternate scenario. All values in this raster must have corresponding entries in the Carbon Pools table."),
            data_type=int,
            units=None,
            projected=True,
        ...
        spec.BooleanInput(
            ...
            about=gettext("Run sequestration analysis. This requires inputs of LULC maps for both baseline and alternate scenarios. Required if run valuation model is selected."),
            required="do_valuation",
        ),
        spec.SingleBandRasterInput(
            id="lulc_alt_path",
            name=gettext("alternate LULC"),
            about=gettext("A map of LULC for the alternate scenario, which must occur after the baseline scenario. All values in this raster must have corresponding entries in the Carbon Pools table. This raster must align with the Baseline LULC raster. Required if Calculate Sequestration is selected."),
        ...
        spec.CSVInput(
            ...
            about=gettext("A table that maps each LULC code to carbon pool data for that LULC type."),
            permissions="r",
            columns=(
                spec.IntegerInput(
                    id="lucode",
                    name=None,
                    about=gettext("LULC codes from the LULC raster. Each code must be a unique integer."),
                ),
                spec.NumberInput(
                    id="c_above",
                    name=None,
        ...
        spec.NumberInput(
            ...
            about=gettext("The calendar year of the baseline scenario depicted in the baseline LULC map. Must be < alternate LULC year. Required if Run Valuation model is selected."),
            required="do_valuation",
        ...
        spec.NumberInput(
            ...
            about=gettext("The calendar year of the alternate scenario depicted in the alternate LULC map. Must be > baseline LULC year. Required if Run Valuation model is selected."),
            required="do_valuation",
            allowed="do_valuation",
            units=u.year_AD,
            expression="float(value).is_integer()",
        ...
        spec.BooleanInput(
            ...
            about=gettext("Calculate net present value for the alternate scenario and report it in the final HTML document."),
            required=False,
            allowed="calc_sequestration",
        ),
        spec.NumberInput(
            ...
            about=gettext("The present value of carbon. Required if Run Valuation model is selected."),
            required="do_valuation",
            allowed="do_valuation",
        ...
        spec.PercentInput(
            ...
            about=gettext(
                "The annual market discount rate in the price of carbon, which reflects society's preference for immediate benefits over future benefits. Required if Run Valuation model is selected. This assumes that the baseline scenario is current and the alternate scenario is in the future."
            ),
            required="do_valuation",
            allowed="do_valuation",
            units=None,
        ...
        spec.PercentInput(
            ...
            about=gettext("The relative annual change of the price of carbon. Required if Run Valuation model is selected."),
            required="do_valuation",
            allowed="do_valuation",
            units=None,
            expression=None,
    ...
    outputs=[
        spec.FileOutput(
            id="report.html",
            about=gettext("This file presents a summary of all data computed by the model. It also includes descriptions of all other output files produced by the model, so it is a good place to begin exploring and understanding model results. Because this is an HTML file, it can be opened with any web browser."),
            created_if=None,
        ),
        spec.SingleBandRasterOutput(
            id="c_storage_bas.tif",
            about=gettext("Raster showing the amount of carbon stored in each pixel for the baseline scenario. It is a sum of all of the carbon pools provided by the biophysical table."),
            created_if=None,
            data_type=float,
            units=u.metric_ton / u.hectare,
        ),
        spec.SingleBandRasterOutput(
            ...
            about=gettext("Raster showing the amount of carbon stored in each pixel for the alternate scenario. It is a sum of all of the carbon pools provided by the biophysical table."),
        ...
        spec.SingleBandRasterOutput(
            ...
            about=gettext("Raster showing the difference in carbon stored between the alternate landscape and the baseline landscape. In this map some values may be negative and some positive. Positive values indicate sequestered carbon, negative values indicate carbon that was lost."),
            created_if="lulc_alt_path",
            data_type=float,
            units=u.metric_ton / u.hectare,
        ),
        spec.SingleBandRasterOutput(
            id="npv_alt.tif",
            about=gettext("Rasters showing the economic value of carbon sequestered between the baseline and the alternate landscape dates."),
            created_if="lulc_alt_path",
            data_type=float,
        ...
        spec.DirectoryOutput(
            ...
            contents=[
                spec.SingleBandRasterOutput(
                    id="c_above_bas.tif",
                    about=gettext("Raster of aboveground carbon values in the baseline scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_above_alt.tif",
                    about=gettext("Raster of aboveground carbon values in the alternate scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_below_bas.tif",
                    about=gettext("Raster of belowground carbon values in the baseline scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_below_alt.tif",
                    about=gettext("Raster of belowground carbon values in the alternate scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_soil_bas.tif",
                    about=gettext("Raster of soil carbon values in the baseline scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_soil_alt.tif",
                    about=gettext("Raster of soil carbon values in the alternate scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_dead_bas.tif",
                    about=gettext("Raster of dead matter carbon values in the baseline scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
                spec.SingleBandRasterOutput(
                    id="c_dead_alt.tif",
                    about=gettext("Raster of dead matter carbon values in the alternate scenario, mapped from the Carbon Pools table to the LULC."),
                    created_if=None,
                    data_type=float,
                    units=u.metric_ton / u.hectare,
                ),
